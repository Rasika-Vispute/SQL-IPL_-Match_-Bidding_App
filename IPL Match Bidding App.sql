...
desc ipl_user;
 ...
 desc ipl_stadium;
 alter table ipl_stadium modify stadium_name varchar(100) unique not null;
 
desc ipl_team;
alter table ipl_team modify team_name varchar(50) unique not null;

desc ipl_player;
alter table ipl_player modify player_name varchar(50) unique not null;

desc ipl_tournament;
alter table ipl_tournament modify from_date date;
alter table ipl_tournament modify to_date date;

desc ipl_match;
alter table ipl_match add constraint fk_match foreign key (Team_id1) references ipl_team(Team_id);
alter table ipl_match add constraint fk_match1 foreign key (Team_id2) references ipl_team(Team_id);

desc ipl_match_schedule;
alter table ipl_match_schedule add constraint fk_ma_schedule foreign key (TOURNMT_ID) references ipl_tournament(TOURNMT_ID);
alter table ipl_match_schedule add constraint fk_ma_schedule1 foreign key (match_ID) references ipl_match(match_ID);


update ipl_match_schedule set start_time = concat_ws("",substr(start_time,4,5),':00');
alter table ipl_match_schedule modify start_time time;
alter table ipl_match_schedule add constraint fk_ma_schedule2 foreign key (stadium_ID) references ipl_stadium(stadium_ID);

select from_date from ipl_tournament where tournmt_id = '2017' or tournmt_id ='2018';
alter table ipl_match_schedule modify match_date date;
alter table ipl_match_schedule modify match_date date;

desc ipl_bidder_details;
alter table ipl_bidder_details modify contact_no bigint not null;
...
desc ipl_bidding_details;
desc ipl_match_schedule;
alter table ipl_bidding_details add constraint fk_bid_det foreign key(bidder_id) references ipl_bidder_details(bidder_id);
alter table ipl_bidding_details add constraint fk_bid_det1 foreign key(schedule_id) references ipl_match_schedule(schedule_id);
alter table ipl_bidding_details drop primary key;
alter table ipl_bidding_details add primary key(bidder_id,schedule_id,bid_team,bid_date);
alter table ipl_bidding_details drop foreign key fk_bid_det,
drop foreign key fk_bid_det1;
alter table ipl_bidding_details modify bid_date datetime default now();

desc ipl_bidder_points;
alter table ipl_bidder_points add constraint foreign key(bidder_id) references ipl_bidder_details(bidder_id); 
alter table ipl_bidder_points add constraint foreign key(TOURNMT_ID) references ipl_tournament(TOURNMT_ID); 
alter table ipl_bidder_points modify total_points int not null default 0;

desc ipl_team_standings; 
alter table ipl_team_standings drop primary key;
alter table ipl_team_standings add constraint fk_ipl_stand foreign key(team_id) references ipl_team(team_id); 
alter table ipl_team_standings add primary key (team_id,TOURNMT_ID);
alter table ipl_team_standings add constraint fk_ipl_stand1 foreign key(TOURNMT_ID) references ipl_tournament(TOURNMT_ID); 
alter table ipl_team_standings modify MATCHES_PLAYED int not null default 0;
alter table ipl_team_standings modify MATCHES_won int not null default 0;
alter table ipl_team_standings modify MATCHES_lost int not null default 0;
alter table ipl_team_standings modify MATCHES_tied int  default 0;
alter table ipl_team_standings modify no_result int  default 0;
alter table ipl_team_standings modify total_points int not null default 0;
...
with temp(bidder_id , count_win ) as 
(select bidder_id , count(bid_status)  as count_win
 from ipl_bidding_details where bid_status = 'won' group by bidder_id )
select  d.bidder_id, ifnull(count_win,0)  , count(bid_status) total_count, (ifnull(count_win,0)/count(bid_status))*100 as win_percent
 from ipl_bidding_details  d left join temp t on d.bidder_id = t.bidder_id where bid_status in
('won','lost','bid') group by d.bidder_id order by win_percent desc;

/* AS we can see in output that bidder 103,104 having 100% winning percentage it means they predicted accureate team which won the match.
so thats why they won all of 5 matches on which they placed bet.
*/

# question  2.	Display the number of matches conducted at each stadium with the stadium name and city.
select stadium_id,count(match_date),stadium_name,city 
from ipl_match_schedule join ipl_stadium using(stadium_id) 
group by stadium_id order by stadium_id;
   
/* Every year each stadium hosts different number of matches, so we have dataset of ipl 2017,2018.
so we can see in output that in year of 2017 and 2018 'Wankhede Stadium' mumbai hosted 18 matches which is maximum from that season
and 'Feroz Shah Kotla','Eden Gardens' hosted equal number of 13 matches in that 2 years.
also capacity of wankhede stadium is 33,500 peoples,'Feroz Shah Kotla' is 41,842 and 'Eden Gardens' is 68,000. 
*/

# question 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?alter
with temp as
(select stadium_id,count(match_id) match_toss_win from ipl_match m join ipl_match_schedule using(match_id)
 join ipl_stadium using(stadium_id)
 where toss_winner = match_winner and status = 'completed' group by stadium_id)
 select stadium_id,stadium_name,match_toss_win,count(match_id) total_match,round(match_toss_win/count(match_id)*100,2) win_percent 
 from ipl_match m join ipl_match_schedule using(match_id)
 join ipl_stadium using(stadium_id)  join temp using(stadium_id) where status = 'completed' group by stadium_id;
 
 /* As we can see in output at 'M. Chinnaswamy Stadium' total 13 matches played and from that 13 matches only 5 teams won matches who won toss 
 in that match..so we can conclude that toss decision dosent matters in 'M. Chinnaswamy Stadium'.so we can say that pitch is suitable for both
 batting and bowling.
 but we can see 'Wankhede Stadium' and 'Sawai Mansingh Stadium' toss_win percentage is nearly 70.00 so we can conclude that toss decision is
 very important at that stadiums. */
 
# question 4.Show the total bids along with the bid team and team name.
select e1.bidder_id,bid_team,no_of_bids,e3.team_name from ipl_bidding_details  e1 join ipl_bidder_points e2 using(bidder_id)
join ipl_team e3 on e1.bid_team = e3.team_id order by no_of_bids desc;

/* we can see here that bidder id 121 placed more number of bids as compared to other bidders,
and he placed bet on 'Kings XI Punjab' 3 times and 'Chennai Super Kings' 2 times
after that bidder 106 also placed on 10 matches.
*/

# question 5.	Show the team id who won the match as per the win details.
select team_id , team_name , team_id1 , team_id2  , match_winner,win_details 
from ipl_team t cross join ipl_match m where instr(win_details,t.remarks);

/* Here we can see winning teams as per win details provided.
*/

# question 6.	Display total matches played, total matches won and total matches lost by the team along with its team name.
select team_name,matches_played,matches_won,matches_lost,tournmt_id,e1.remarks from ipl_team_standings e1 join ipl_team e2 using(team_id);

/* As we can see here that, mumbai indians won ipl 2017 season by winning 10 matches out of 14 matches,whereas 2018 ipl season 
was won by csk by winning 9 matches out of 14 matches.
and we can say that RCB had the worst season of all as it was at the last positon by winning only 3 matches out of 14 matches in season 2017,
and similarly delhi daredevils was at last position by winning only 5 matches from 14 matches in ipl 2018.
*/

# question 7.	Display the bowlers for the Mumbai Indians team.
select player_id,player_name as bowlers_MI from ipl_player where player_id in
(select player_id from ipl_team_players  where player_role = 'bowler' and team_id =
(select team_id from ipl_team where remarks = 'mi'));

/* we can see all bowlers from mumbai indians here, it includes fast bowlers, spinners, part-time bowlers.
*/

/* question 8.	How many all-rounders are there in each team, Display the teams with more than 4 all-rounders in descending order.*/
select team_id,team_name,count(player_id) all_rounders
from ipl_team_players join ipl_team using(team_id) 
where player_role = 'All-Rounder' group by team_id having count(player_id)>4 order by count(player_id) desc;

/* we can see here that delhi daredevils and kings XI punjab has 7 all-rounders which is greater than all of other teams.
so captains of delhi daredevils and kings XI punjab have more numbers of options for bowling,if any bowler is injured or
if any bowler is not performing well thus he can give bowling to other bowler or all-rounder and because of more number
of all-rounders batting line-up for that team is so deep so top-order batsman can easily play with more strike-rate..
*/

/* question 9. Write a query to get the total bidders points for each bidding status of those bidders who bid on CSK when it won the match in M. Chinnaswamy Stadium bidding year-wise.
Note the total bidders’ points in descending order and the year is bidding year.
Display columns: bidding status, bid date as year, total bidder’s points */
select bidder_id,stadium_name,schedule_id,year(bid_date),total_points,bid_status,match_id 
from ipl_stadium join ipl_match_schedule using(stadium_id) 
join ipl_bidding_details using(schedule_id)
join ipl_bidder_points using(bidder_id)
join ipl_match using(match_id)
where stadium_name like '%chinna%' and bid_team = 1 and win_details like '%csk%' order by total_points desc;

/* Here we can see that only 2 bidders placed bid on csk and csk won that match in 'M. Chinnaswamy Stadium',
total points for bidder_id 104 is 17, but status for bidder_id 102 is not updated yet so thats why points for
that bidder showing zero here once the status of bid gets updated then points of that bidder will be display.
*/

/* question 10.	Extract the Bowlers and All Rounders those are in the 5 highest number of wickets.
Note 
...
select * from
(select *,rank() over(order by cast(substr(PERFORMANCE_DTLS,(instr(PERFORMANCE_DTLS,'w')+4),(instr(PERFORMANCE_DTLS,'d')-1 - instr(PERFORMANCE_DTLS,'w')-3))  as float) desc) top_5 from
(select *,cast(substr(PERFORMANCE_DTLS,(instr(PERFORMANCE_DTLS,'w')+4),(instr(PERFORMANCE_DTLS,'d')-1 - instr(PERFORMANCE_DTLS,'w')-3))  as float) wick
from ipl_player where player_id in
(select player_id  from ipl_team_players where player_role = 'bowler' or player_role = 'all-rounder'))abc)xyz
where top_5 < 6;

/* Here we can see that andre tye takes 24 wickets which is highest number of wickets than other bowlers/all-rounders
and he represents australia in international matches and he played for CSK for that season.
at 2nd position,siddharth kaul takes 21  wickets and he is one of the successful fast-bowler of indian cricket team
and he played for Rajasthan Royals for that season. 
*/

# question 11.	show the percentage of toss wins of each bidder and display the results in descending order based on the percentage
with temp as (select bidder_id ,count(bid_team) as count  from ipl_bidding_details group by bidder_id)
select bidder_id ,count(toss_winner_id)/count * 100 as percent from 
(select bidder_id, 
case when toss_winner = 1 and team_id1 = team_id then team_id1 when toss_winner = 2 and team_id2 = team_id then team_id2
when toss_winner = 1 and team_id1 <> team_id then team_id1 when toss_winner = 2 and team_id2 <> team_id then team_id2 end 
as toss_winner_id
 , bid_team 
from ipl_match m join ipl_match_schedule using (match_id) join ipl_bidding_details using(schedule_id) 
join ipl_team t where instr(win_details,t.remarks)) abc join temp using(bidder_id)
where toss_winner_id = bid_team
group by bidder_id order by count(toss_winner_id)/count * 100 desc;

/* we can see that bidder_id 110,118 and 124 predicted very accurately for toss winning.
because decision of batting first or bowling first are more important in cricket game,
because we can choose batting or bowling by taking pitch condition in mind.
so if pitch condition is good for batting then toss winner always takes bowling first so they
can easily chase score while batting second.
*/


# question 12.	find the IPL season which has min duration and max duration.
select * from ipl_tournament;
desc ipl_tournament;
select TOURNMT_NAME,datediff(to_date,from_date) duration from ipl_tournament where datediff(to_date,from_date)=
(select min(datediff(to_date,from_date)) from ipl_tournament)
union
select TOURNMT_NAME,datediff(to_date,from_date) from ipl_tournament where datediff(to_date,from_date)=
(select max(datediff(to_date,from_date)) from ipl_tournament);

/* we can see here ipl season 2009 just completed in 36 days,it was second season of ipl history and on that season
tournament held in South Africa so thats why they completed tournament in just 36 days.
we can see here also that season 2012 and 2013 completed in 53 days.
*/

/* question 13.	Write a query to display to calculate the total points month-wise for the 2017 bid year. 
...
*/
select bidder_id,bidder_name,year(bid_date),month(bid_date),total_points 
from ipl_bidding_details join ipl_bidder_points using(bidder_id) 
join ipl_bidder_details using(bidder_id)
where year(bid_date)='2017' group by bidder_id,month(bid_date) order by total_points desc,month(bid_date);

/* Here we can see that bidder Aryabhatta Parachure got more number of points than all of other bidders.
and he got 35 total points.
after him bidder_id 103 and 104 ar at the top 2nd and 3rd position with 19 and 17 points. 
*/

# question 14.	Write a query for the above question using sub queries by having the same constraints as the above question.
select * from ipl_bidder_details;
...
select bidder_id , bidder_name , year(bid_date) as year , month(bid_date)  as month  , x.total_points
from ipl_team_standings join ipl_bidder_points x using(tournmt_id) 
join ipl_bidder_details  using(bidder_id) 
join ipl_bidding_details using(bidder_id) 
where year(bid_date) = 2017
group by bidder_id , month(bid_date)
order by x.total_points desc , month(bid_date) asc;

/*# question 15.	Write a query to get the top 3 and bottom 3 bidders based on the total bidding points for the 2018 bidding year.
...
*/
(select bidder_id , total_points , bidder_name , rank() over(order by total_points desc) ranks
from ipl_bidding_details join ipl_match_schedule using(schedule_id) 
join ipl_bidder_points using(bidder_id) 
join ipl_bidder_details using(bidder_id)
...
/* Here we can see TOP 3 bidders from season 2018 with bidder_id 121 has got 35 which is highest among all of other bidders
who placed bid on that season.After him bidder_id 103 and 104 are at the 2nd and 3rd position with 19 and 17 points. 
and  we can see Bottom 3 bidders from season ,they placed bid on teams and unfortunately that team lost match
thats why they got zero points.
*/


/* question 16.	Create two tables called Student_details and Student_details_backup.
...
CREATE TABLE gl_students (
    ...
    email VARCHAR(30),
    mobile_number CHAR(30)
);

CREATE TABLE gl_students_bk (
    student_id INT,
    student_name VARCHAR(30),
    email VARCHAR(30),
    mobile_number CHAR(30)
...
SELECT * FROM gl_students;
SELECT * FROM gl_students_bk;

CREATE TRIGGER  tg_1
 BEFORE INSERT ON gl_students FOR EACH ROW 
    INSERT INTO gl_students_bk VALUES (new.student_id , new.student_name , new.email , new.mobile_number);

SELECT * FROM gl_students;

insert into gl_students values (1,'Jayesh Narkhede','@gmail.com',2564896543),
(2,'Anadi','ana@gmail.com',2564836543),
(3,'shubham','shubh@gmail.com',2562396543);

SELECT * FROM gl_students_bk;

UPDATE gl_students SET mobile_number = 1234567890
WHERE student_id = 1;

SELECT * FROM gl_students;
SELECT * FROM gl_students_bk;

## here as we have update the mobile number but there is no change in the backup table as the bakup should not be changed 
## even if there is change in the main table.

insert into gl_students values (4,'mohit','30@gmail.com',2564896545),
(5,'sajraj','saj@gmail.com',2564836546),
(6,'viraj','viju@gmail.com',2562396547);

SELECT *FROM gl_students_bk;

insert into gl_students values
(7,'karuna','karu@gmail.com',2560111789),
(8,'shantanu','hantanu@gmail.com',5641239870),
(9,'mitali','mit@gmail.com',3214569875),
(10,'anushka','anuka@gmail.com',1478529630);
